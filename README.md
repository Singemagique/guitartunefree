...
- **Strum check (beta)** — strum all strings once and see every string's offset at a glance: per-string cents bars, in-tune checks at ±5¢, and honest "couldn't confirm" states. Classical DSP (spectral template matching with inharmonicity fitting), analysed off the main thread. Refuses to guess when the whole guitar reads a semitone off (capo mismatch) and sits out octave-paired tunings for now. The precision Single-mode tuner is untouched.
...
**Quick install:** grab `TrueString-v2.0.0-debug.apk` from the [latest release](https://github.com/Singemagique/guitartunefree/releases/latest) (or the `truestring-debug-apk` artifact on any [Android CI run](https://github.com/Singemagique/guitartunefree/actions/workflows/android.yml)).
...
Real-guitar calibration of the strum check (recordings wanted — see [research/recordings](research/recordings/README.md)); roadmap in [ROADMAP.md](ROADMAP.md).
